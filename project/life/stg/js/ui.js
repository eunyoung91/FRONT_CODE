...
var isApp = strAgent.indexOf("promotewebview") > -1 ? true : false;
//isApp = true;
...
//		$target.toggleClass('in').siblings().find('.modal').removeClass('in');
	...
	// 2020-04-14 ipad issue
	if (navigator.platform == 'iPad'){
		$('.modal-survey .custom-select select').on('focus touchstart', function(){
			$('.modal-survey').css({
				position: 'absolute',
				top: 0,
				bottom: 'auto',
...
function replaceImageLinks(target, basic, change) {
	if(isApp){
		...
		if(agent.indexOf("eduwill_portal") > -1 ){
			appName = "EDUWILL_PORTAL";
		} else if (agent.indexOf("eduwill_study") > -1){
			appName = "EDUWILL_STUDY";
		} else if (agent.indexOf("eduwill_sisa") > -1){
			appName = "EDUWILL_SISA";
		} else {
			appName = "TEST";
		}

		if(agent.indexOf("_ios") > -1){
			appName = appName + "_IOS";
		}		

		$(target + ' img').each(function(){

			var linkType = $(this).parent().attr("target");
			
			var pcLink = $(this).parent().data("pcLink");
			pcLink = pcLink.toLowerCase().indexOf("http") > -1 ? pcLink : "https:" + pcLink;

			var mobileLink = $(this).parent().data("mobileLink");
			mobileLink = mobileLink.toLowerCase().indexOf("http") > -1 ? mobileLink : "https:" + mobileLink;

			if(linkType == "_blank"){

				var mobileApiUrl = "https://s-api.eduwill.net";
				var appUserId = userId == undefined ? "" : userId;
				var appEdw = shaUserId;
				var url = change == "pc" ? pcLink : mobileLink;

				var strFun = "appToWebSSOTokenOpen('" + mobileApiUrl + "','" + appUserId + "','" + appName + "','" + appEdw + "','" + url + "')";

				$(this).parent().attr("href", "#");
				$(this).parent().attr("onclick", strFun);
			}else{
