...
(function(myroom){
	...
	var Player = (function(){
		...
		function Player(){
			...
			this.version			= "1.0.0";
			this.default_domain		= "d-myroom.eduwill.net";
			this.service_domain		= "";
			this.api_encript_url	= "/api/player/dataEncript";
			this.api_nPlayer_url	= "/class/player/nPlayer";
			this.api_nManager_url	= "/class/player/nManager";
			...
			if(currUrlArray.length >= 2){
				...
				if(currUrlArray[0].indexOf(this.default_domain) == -1){
					this.service_domain += this.default_domain;
				}else{
					this.service_domain += currUrlArray[0];
				}
			}
		}
		
		// 강의정보를 전송하여 암호화데이터 생성;
		Player.prototype.dataEncript = function(progressCode, productCode, lectureCode, lectureSeq, siteCode){
			var param = {"progressCode":progressCode,"productCode":productCode, "lectureCode":lectureCode, "lectureSeq":lectureSeq, "siteCode":siteCode};
			var jsonData = this.ajax.postSyncAjax(this.service_domain + this.api_encript_url, param);
		...
		Player.prototype.nManagerLaunch = function(encriptData, siteCode, position){
			...
			this.ajax.postAjax(this.service_domain + this.api_nManager_url, param, this.successCallback, this.failCallback);
		};
		...
		Player.prototype.nPlayerLaunch = function(encriptData, siteCode, div, position){
			...
			if(encriptData == undefined || encriptData == ""){
				...
				var url = this.service_domain + this.api_nPlayer_url + "?rate=1&div=" + div + "&siteCode=" + siteCode + "&position=" + position + "&agent=" + Player.Agent + "&encriptData=";			
				var wSize = 1337;
				var hSize = 681;
				var strTitle = "에듀윌 강의실";
				
				// 독한에듀윌[독한모드]
				hSize = siteCode == this.site_dokhan ? hSize - 53 : hSize;
				
				// IE
				if ("ActiveXObject" in window) {
					var mf = window.open((url + encriptData),'AquaNPlayer','left=0, top=0, width=' + wSize + ', height=' + hSize + ',toolbar=no, menubar=no, location=no, directories=no, resizable=yes, status=no, scrollbars=no');
						mf.focus();
				...
				// IE 외
					...
					var href = "nplayer://launch?" + params + "url=" + encodeURIComponent("http:" + url) + encriptData;
		...
		// Method
		Player.prototype.run = function(progressCode, productCode, lectureCode, lectureSeq, siteCode, div, position){
			
			//암호화 데이터 생성
			var encriptData = this.dataEncript(progressCode, productCode, lectureCode, lectureSeq, siteCode);
			if(encriptData != ""){
				if(Player.Agent == "pc" || Player.Agent == "mac"){
...
/*
 * 영상 플레이어 실행모듈
 * @Function                : playerRun(progressCode, productCode, lectureCode, lectureSeq, siteCode, div, bookmarkPosition)
 * @param progressCode      : 과정코드
...
function playerRun(progressCode, productCode, lectureCode, lectureSeq, siteCode, div, bookmarkPosition){
	...
	_player.run(progressCode, productCode, lectureCode, lectureSeq, siteCode, div, bookmarkPosition);
}
...
// 각 코드로 암호화된 데이터 생성
function dataEncript(progressCode, productCode, lectureCode, lectureSeq, userId, siteCode){
	var src = "/class/player/dataEncript2?progressCode=" + progressCode + "&productCode=" + productCode + "&lectureCode=" + lectureCode + "&lectureSeq=" + lectureSeq + "&userId=" + userId + "&siteCode=" + siteCode;
