...
$(document).ready(function () {
	...
	//쿠키를 이용한 결과 토스트 메시지 - decode
	var warningMsgEnc = getCookie_dec("warningMsgEnc");
	if(warningMsgEnc) {
		fnc_toastMsgForWarning(warningMsgEnc);
		delCookie("warningMsgEnc");
	}
...
function fnc_ajaxErrorHandle(data, status, err) {
    ...
    if (needLogin) {
		...
		if(data.status == 403) {
			bAlert(data.responseText).done(function() { return false; });
		}else {
...
function getCookie_dec(name) {
    ...
    for (var i = 0; i < ca.length; i++) {
        ...
        if (c.indexOf(name) == 0) {
            return decodeURIComponent(c.substring(name.length, c.length).replace(/\+/g, '%20'));
        }
    }
    return "";
}
