...
<%
	...
	If gLoginMemKind = "5" Then '관리자
		...
		AlertAndPageMove_PAGE = "/campus_common/" & g_TP_YEAR & "/" & g_TP_YEAR & "_teamplay_step/index.asp"
	ElseIf g_JungsiResultDateCheck <> "Y" Then
		AlertAndPageMove_MSG = "팀플장학 발표 기간이 아닙니다."
		AlertAndPageMove_PAGE = "/campus_common/" & g_TP_YEAR & "/" & g_TP_YEAR & "_teamplay_step/index.asp"
	ElseIf gAmsMcodeCnt > 0 Then '재원생 인증을한 학생
		...
		ElseIf g_JungsiResultTargetCheck <> "Y" Then
			AlertAndPageMove_MSG = "팀플장학 지급에 선정된 대상자가 아닙니다."
			AlertAndPageMove_PAGE = "/campus_common/" & g_TP_YEAR & "/" & g_TP_YEAR & "_teamplay_step/index.asp"
		End If
	Else
...
<head>
	...
	<link rel="stylesheet" type="text/css" href="teamplayApply.css">
						...
						<div class="con_wrap">
								<div class="teamplay_apply result pt60">
									<form name="form_proc_my_univ" id="form_proc_my_univ" method="post" action="">
										...
										<div class="inner"> 
											<div class="inner_title" style="padding-top:125px;">
												<h2>
													<img src="<%=Application("img_path")%>/library/campus_common/2025/2025_teamplay_apply/con01_title.png" alt="2차 서류 제출 안내">
												</h2>
												<div class="img_wrap mt80">
													<img src="<%=Application("img_path")%>/library/campus_common/2025/2025_teamplay_apply/con01_sub_title.png" alt="제출기간">
												</div> 
											</div>

											<div class="apply_register mt50">
												<div class="reg_content">
													<div class="img_wrap" style="padding-top:100px;">
														<img src="<%=Application("img_path")%>/library/campus_common/2025/2025_teamplay_apply/con01_sub_title02.png" alt="장학금 지급 서류 등록">
													</div> 
													<div class="img_wrap">
													...
													<div class="teamplay_img_wrap2" id="inputArea1">
														<img src="<%=Application("img_path")%>/library/campus_common/2025/2025_teamplay_apply/sec_apply_title01.png" alt="최종 진학대학 합격증명서 등록"/>
													</div>
													<%
														'최종 진학대학 가져오기 S
														strSql = ""
														...
														Else
															DU_UNIV_CD = 0
															DU_UNIV_NAME = "합격 내역이 존재하지 않습니다"
															DU_COLLECT_UNIT = "합격 내역이 존재하지 않습니다"
																...
																<td>
																	<div class="input_file univ">
																		...
																		<div class="input_wrap">
																			<span class="title_in">대학 </span>
																			<span class="ph_in">진학대학 정보를 입력해 주세요</span>
																			<!-- <%=DU_UNIV_NAME%> -->
																			<br>
																			<span class="title_in">학과 </span>
																			<span class="ph_in">진학학과 정보를 입력해 주세요</span>
																			<!-- <%=DU_COLLECT_UNIT%> -->
																		</div>
																		...
																		<%
																			...
																			If CSTr(GetServerGubun()) = "dev" Then '개발서버 체크
																				SUSI_UNIV_TIME_S = "2023"&"1201"&"100000" '수시합격 입력 시작일
																				SUSI_UNIV_TIME_E = "2024"&"0102"&"235959" '수시합격 입력 마감일
																			Else
																				SUSI_UNIV_TIME_S = "2023"&"1226"&"100000" '수시합격 입력 시작일
																				SUSI_UNIV_TIME_E = "2024"&"0102"&"235959" '수시합격 입력 마감일
																			End If
		
																			If CDbl(SUSI_UNIV_TIME_S) <= CDbl(GetNowDate()) And CDbl(GetNowDate()) <= CDbl(SUSI_UNIV_TIME_E) Then
																				passUnivType = "susi"
																			Else
													...
													<div class="teamplay_img_wrap2" id="inputArea2">
														<img src="<%=Application("img_path")%>/library/campus_common/2025/2025_teamplay_apply/sec_apply_title02.png" alt="계좌정보 및 통장사본 등록"/>
													</div>
															...
															<tr>
																...
																<td class="taL">
																	...
																	<div class="input_file account">
																		<input type="text" name="TP_MEMBANK_UNM" id="TP_MEMBANK_UNM" value="" size="10" maxlength="10" style="width:130px;" placeholder="예금주명">
															...
															<tr>
																...
																<td>
																	<div class="input_file account">
																		...
																		<p class="info_txt">
																			&middot; 10MB 이하의 jpg, jpeg, gif, png 파일만 등록 가능<br>
																			&middot; 본인 계좌 정보에 입력한 정보와 일치하는 본인 통장사본 등록<br>
																			&middot; 은행명, 예금주명, 계좌번호가 확인 가능한 이미지 등록
																		</p>
																	</div>
																</td>
															</tr>
											...
											<div class="apply_register register02">
												<div class="reg_content" style="padding-top:80px;">
													<h2 id="inputArea3">
														<img src="<%=Application("img_path")%>/library/campus_common/2025/2025_teamplay_apply/con02_title.png" alt="장학금 지급 안내">
													</h2>
													<div class="txtc mt55">
														<strong class="payment_info_txt">
															메가스터디교육㈜(이하 ‘회사’)는 메가스터디학원 <em class="blt_star">*</em>팀플장학생으로 선정된 <strong><%=gLoginMemName%></strong>(이하 “장학생”)에게,<br>
															장학생이 입학한 대학을 통하여 2025년 5월경에 장학금 100만원을 지급합니다.<br>
											...
											<div class="apply_register register03">
												<div class="reg_content" style="padding-top:100px;">
													<h2 id="inputArea3">
														<img src="<%=Application("img_path")%>/library/campus_common/2025/2025_teamplay_apply/con03_title.png" alt="개인정보 수집·이용·취급 위탁 동의">
													</h2>
													
													<h3 class="box_title">장학금 지급을 위한 개인정보 제3자 제공 동의</h3>
													<div class="ag_box account mt20">
														<strong>개인정보의 제3자 제공</strong><br>
														개인정보 보호법 제17조 제4항에 따라 정보주체에게 불이익이 발생하는지 여부, 암호화 등 안전성 확보에 필요한 조치 등을<br>
														고려하여 정보주체의 동의없이 개인정보를 제공합니다.
														<ul class="list_txt mt40">
															<li><strong>[제공받는자]</strong> 장학금 지급 대상자의 입학 대학</li>
															<li><strong>[제공항목]</strong> 이름, 생년월일, 학번, 예금주명 및 계좌번호(통장사본 포함)</li>
															<li><strong>[제공목적]</strong> 입학한 대학을 통해 “메가스터디교육㈜”이 장학생에게 장학금 지급을 위함 </li>
															<li><strong>[보유 및 이용기간]</strong> <span style="text-decoration: underline;">1년</span></li> 
														</ul>
													</div> 
												</div>
											</div>
										...
										<div class="bottom_btn_box" style="padding-top:180px;">
											<!-- 활성화일 경우 class on -->
	...
	<script type="text/javascript">
		...
		function fnResultCommit() {
			...
			if(TpCompleteCheck("alert") == true) {			
				if (confirm("최종 제출한 후에는 수정이 불가능합니다.\n최종 제출하시겠습니까?")) {
		...
		var TpCompleteCheck = function(type) {
			...
			if($("#TP_UNIVPASSFILE_YN").val() != "Y") {
				if(type == "alert"){
					alert("최종 진학대학 합격증명서을/를 모두 등록해 주세요.");
					fnScrollMove("#inputArea1");
				}
				return false;
			}
			...
			if ($(':radio[name="private_info_agree1"]:checked').val() == "N") {
				if(type == "alert"){
					alert("장학금 지급을 위한 개인정보 수집-이용-취급 위탁에 동의하셔야만 최종 제출이 가능합니다.");
