...
<%
	...
	If CSTr(GetServerGubun()) = "dev" Then '개발서버 체크
		F_applyStartTime = "2024"&"1120"&"100000" '정시 장학생 지원 시작일
		F_applyEndTime = "2025"&"0105"&"235959" '정시 장학생 지원 마감일
	Else
		F_applyStartTime = "2024"&"1205"&"100000" '정시 장학생 지원 시작일 실서버 테스트로 임시 오픈. 
		F_applyEndTime = "2025"&"0105"&"235959" '정시 장학생 지원 마감일
	End If

	'발표 일정 / 2차 서류 제출
	If CSTr(GetServerGubun()) = "dev" Then '개발서버 체크
		F_resultStartTime = "2024"&"1231"&"100000" '정시 장학생 발표 시작일
		F_resultEndTime = "2025"&"0228"&"235959" '정시 장학생 발표 마감일
	Else
		F_resultStartTime = "2025"&"0106"&"100000" '정시 장학생 발표 시작일
		F_resultEndTime = "2025"&"0228"&"235959" '정시 장학생 발표 마감일
	End If
	...
	If gLoginOk Then
		...
		If IsDevSvr() = True Then '테스트서버 <-> 실서버 과정 코드가 달라서 분기 필요
			strSql = strSql & vbCrlf & "				WHEN GH_ACA_CD IN (27, 44, 38, 121, 12) THEN 1897" '2024 재종 10월 과정 : 신촌, 양지기숙, 서초기숙, 안성기숙, 서초
			strSql = strSql & vbCrlf & "				WHEN GH_ACA_CD IN (11, 17, 41, 115, 74, 75, 76, 45, 119) THEN 1898" '2024 재종 11월 과정 : 강남, 서초, 강북,  노량진, 송파, 부천, 분당, 일산, 평촌, 테스트
		Else
			strSql = strSql & vbCrlf & "				WHEN GH_ACA_CD IN (27, 44, 38, 121, 12) THEN 1886" '2024 재종 10월 과정 : 신촌, 양지기숙, 서초기숙, 안성기숙, 서초
			strSql = strSql & vbCrlf & "				WHEN GH_ACA_CD IN (11, 17, 41, 115, 74, 75, 76, 45, 119) THEN 1887" '2024 재종 11월 과정 : 강남, 강북,  노량진, 송파, 부천, 분당, 일산, 평촌, 테스트
		End If
	...
	' 		F_applyStartTime = "2024"&"1205"&"100000" '정시 1차 서류제출 시작일
	' 		F_applyEndTime = "2025"&"0105"&"235959" '정시 1차 서류제출 마감일
	' End Select
